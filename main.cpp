...
void expand(
	vector<Army> & newPureArmies,
	vector<Army> & newHeroArmies,
	const vector<Army> & oldPureArmies,
	const vector<Army> & oldHeroArmies, 
	const size_t currentArmySize,
	...
	size_t firstDominance)
{
	...
	if (armySize == (instance.maxCombatants - 1) && optimizable)
		{ // Must be optimizable and the last expansion
	...
	for (i = 0; i < pureMonsterArmiesSize; i++)
		...
		for (j = i+1; j < pureMonsterArmiesSize; j++)
			...
			else if (*currentFightResult <= pureMonsterArmies[j].lastFightData)
				{
				// currentFightResult has more followers implicitly 
	...
	// prepare monsterlist
	// Normal Monsters are true by default
	for (i = 0; i < leftMonsterSetSize; i++)
		leftMonsterSet[i] = monsterReference[i].rarity != NO_HERO; 
	...
	for (i = 0; i < heroMonsterArmiesSize; i++)
		...
		if (!currentFightResult->dominated)
			...
			for (j = i+1; j < heroMonsterArmiesSize; j++)
				...
				else if (*currentFightResult <= heroMonsterArmies[j].lastFightData)
					{
					// i has more followers implicitly
					usedHeroSubset = true; // If j doesn't use a strict subset of the heroes i used, it cannot dominate i
					for (sj = 0; sj < armySize; sj++)
						{
						// for every hero in j there must be the same hero in i
	...
	if (instance.targetSize <= instance.maxCombatants)
		...
		for (size_t i = 0; i < instance.maxCombatants; i++)
			...
			for (size_t m = 0; m < availableMonsters.size(); m++)
				...
				if (!tempArmy.lastFightData.rightWon || (tempArmy.lastFightData.monstersLost > (int) i && i+1 < instance.maxCombatants))
					{
...
void solveInstance(Instance & instance, size_t firstDominance)
{
	...
	// Ignore checking for Hero Cost
	for (i = 0; i < availableHeroes.size(); i++)
	...
	if (optimizable)
		{
		// Check with normal Mobs
		for (i = 0; i < pureMonsterArmies.size(); i++)
			{
			simulateFight(pureMonsterArmies[i], tempArmy);
			if (!pureMonsterArmies[i].lastFightData.rightWon)
				{
				// Monster won the fight
				optimizable = false;
				break;
				}
			}
	...
	if (optimizable)
		...
		for (i = 0; i < heroMonsterArmies.size(); i++)
			...
			if (!heroMonsterArmies[i].lastFightData.rightWon)
				{
				// Hero won the fight
				optimizable = false;
				break;
	...
	for (size_t armySize = 1; armySize <= instance.maxCombatants; armySize++)
		...
		if (!instance.hasWorldBoss && instance.bestSolution.monsterAmount > 0 && instance.bestSolution.followerCost == 0)
			break;
		...
		if (armySize < instance.maxCombatants)
			...
			if (armySize == firstDominance)
				...
				if (!iomanager.askYesNoQuestion("Continue calculation?", DETAILED_OUTPUT, POSITIVE_ANSWER))
					return;
...
int main(int argc, char** argv)
{
	...
	do
		...
		if (iomanager.outputLevel == CMD_OUTPUT)
			{
			// TODO: Reset Output Level if userwantscontinue
			if (instances.size() > 1)
