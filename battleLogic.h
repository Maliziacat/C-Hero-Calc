...
inline void ArmyCondition::startNewTurn()
{
	...
	for (i = monstersLost; i < armySize; i++)
		...
		case PROTECT:
			if (skillTargets[i] == ALL || skillTargets[i] == lineup[monstersLost]->element)
...
inline void ArmyCondition::getDamage(const int turncounter, const Element opposingElement)
{
	...
	case RAINBOW:
		if (rainbowConditions[monstersLost] == VALID_RAINBOW_CONDITION)
			turnData.buffDamage += (int) skillAmounts[monstersLost];
		break;

	case ADAPT: 
		if (opposingElement == skillTargets[monstersLost])
			turnData.multiplier *= skillAmounts[monstersLost];
		break;

	case BERSERK:
		turnData.multiplier *= (float) pow(skillAmounts[monstersLost], berserkProcs); berserkProcs++; 
		break;
	...
	case PIERCE:
		...
		break;
	case VALKYRIE:
		turnData.valkyrieMult = skillAmounts[monstersLost]; 
		break;
		}
...
inline void ArmyCondition::resolveDamage(TurnData & opposing)
{
	...
	for (i = frontliner; i < armySize; i++)
		...
		// Aoe that doesnt affect the frontliner
		if (i > frontliner)
		...
		if (remainingHealths[i] <= 0)
			...
			// Avoid overhealing
			if (remainingHealths[i] > lineup[i]->hp)
				remainingHealths[i] = lineup[i]->hp;
			}
...
inline void simulateFight(Army & left, Army & right, bool verbose = false)
{
	...
	if (leftCondition.monstersLost >= leftCondition.armySize)
		{
